...
func setupDatabase(cmd *cobra.Command, d pythonData) {
	if !d.hadDB {
		quickSetup(cmd.Flags(), d)
	}
}

func buildImgService(cmd *cobra.Command) *img.Service {
...
func setupFileCache(cmd *cobra.Command) diskcache.Interface {
...
func setServerRoot(server *settings.Server) {
...
func createSocketListener(cmd *cobra.Command, server *settings.Server) net.Listener {
	listener, err := net.Listen("unix", server.Socket)
	utils.ExitIfError("Unable to create socket listener, %v", err)

	socketPerm, err := cmd.Flags().GetUint32("socket-perm") //nolint:govet
	utils.ExitIfError("Unable to get socket parameter, %v", err)

	err = os.Chmod(server.Socket, os.FileMode(socketPerm))
	utils.ExitIfError("Permission denied on socket, %v", err)

	return listener
}

func createHttpsListener(server *settings.Server, address string) net.Listener {
	cer, err := tls.LoadX509KeyPair(server.TLSCert, server.TLSKey) //nolint:govet
	utils.ExitIfError("Error loading x509 certificate, %v", err)

	listener, err := tls.Listen("tcp", address, &tls.Config{
		MinVersion:   tls.VersionTLS12,
		Certificates: []tls.Certificate{cer}},
	)
	utils.ExitIfError("Error creating TLS listener, %v", err)

	return listener
}

func createHttpListener(server *settings.Server, address string) net.Listener {
	listener, err := net.Listen("tcp", address)
	utils.ExitIfError("Error creating TCP listener, %v", err)

	return listener
}

func createListenerFromSettings(cmd *cobra.Command, server *settings.Server, address string) net.Listener {
	if server.Socket != "" {
		return createSocketListener(cmd, server)
	}

	if server.TLSKey != "" && server.TLSCert != "" {
		return createHttpsListener(server, address)
	}

	return createHttpListener(server, address)
}
...
func captureOsSignal(listener net.Listener) {
	sigc := make(chan os.Signal, 1)
	signal.Notify(sigc, os.Interrupt, syscall.SIGTERM)
	go cleanupHandler(listener, sigc)
}

func bindToHttpHandler(cmd *cobra.Command, server *settings.Server, d pythonData) http.Handler {
	imgSvc := buildImgService(cmd)
	fileCache := setupFileCache(cmd)
	assetsFs := getAssetsFs()
	handler, err := fbhttp.NewHandler(imgSvc, fileCache, d.store, server, assetsFs)
	utils.ExitIfError("Unable to bind to handles, %v", err)

	return handler
}

func setupServer(cmd *cobra.Command, server *settings.Server, d pythonData) {
	address := fmt.Sprintf("%s:%s", server.Address, server.Port)
	listener := createListenerFromSettings(cmd, server, address)

	// close the listener when done
	captureOsSignal(listener)
	defer listener.Close()

	// create a cmux object
	tcpm := cmux.New(listener)

	// declare the match for different servies required
	grpcListener := tcpm.MatchWithWriters(cmux.HTTP2MatchHeaderFieldSendSettings("content-type", "application/grpc"))
	httpListener := tcpm.Match(cmux.HTTP1Fast())
	http2Listener := tcpm.Match(cmux.HTTP2())

	// http/http2 routing rules
	handler := bindToHttpHandler(cmd, server, d)

	// initialize the servers by passing in the custom listeners
	log.Println("Grpc server started...")
	go grpcServer(grpcListener)

	log.Println("Http server started...")
	go httpServer(httpListener, handler)
	go httpServer(http2Listener, handler)

	log.Printf("Server listenering on %s", address)
...
user created with the credentials from options "username" and "password".`,
	Run: python(func(cmd *cobra.Command, args []string, d pythonData) {
		...
		setupDatabase(cmd, d)
		setupLog(server.Log)
		setServerRoot(server)
		setupServer(cmd, server, d)
